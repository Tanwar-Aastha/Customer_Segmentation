...
from src.utils import save_object_file
...
class CustomFunction:
    def cat_function(cat_data):
        try:
            logging.info("Inside the Cat function")
            ...
            ]), "Other currency", cat_data['Product'])
    ...
    def date_time_function(datetime_data):
        datetime_data['Date received'] = pd.to_datetime(datetime_data['Date received'], format='%d-%m-%Y')
        datetime_data['Date sent to company']=pd.to_datetime(datetime_data['Date sent to company'], format='%d-%m-%Y')
